...
export default function ServicesPage() {
  usePageMeta({
    title: 'Brand Activation, Digital & Design Services | AQUA',
    description: 'Explore AQUA services in Bangladesh across brand activation, events, branding, architecture, digital platforms, automation, content, video and sports marketing.',
    path: '/services',
  });
          ...
          {services.map((service, index) => (
            <article id={service.slug} key={service.slug} className="grid lg:grid-cols-12 gap-6 lg:gap-10 py-8 md:py-10 border-b border-black/15 scroll-mt-32">
              ...
              <div className="lg:col-span-4">
                <h2 className="text-[2rem] md:text-[2.8rem] font-medium tracking-[-0.04em] leading-none">
                  <Link to={`/services/${service.slug}`} className="hover:text-[#0891b2] transition-colors">{service.title}</Link>
                </h2>
              ...
              <div className="lg:col-span-4">
                ...
                <div className="mt-7 flex flex-wrap gap-5">
                  <Link to={`/services/${service.slug}`} className="inline-flex text-[11px] font-bold uppercase tracking-[0.15em] border-b border-black/30 pb-1">Explore service ↗</Link>
                  <Link to="/work" className="inline-flex text-[11px] font-bold uppercase tracking-[0.15em] border-b border-black/30 pb-1">See related work ↗</Link>
                </div>
              </div>
              <Link to={`/services/${service.slug}`} className="lg:col-span-3 aspect-[4/3] overflow-hidden rounded-xl bg-black/5" aria-label={`Explore AQUA ${service.title} service`}>
